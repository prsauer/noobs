...
bool draw_source_outline(obs_scene_t *scene, obs_sceneitem_t *item, void *p) {
  ...
  // Top border
  gs_matrix_push();
  gs_matrix_translate3f(pos.x + (crop.left * scale.x), pos.y + (crop.top * scale.y), 0.0f);
  gs_draw_sprite(nullptr, 0, width, 4.0f);
  gs_matrix_pop();

  // Bottom border
  gs_matrix_push();
  gs_matrix_translate3f(pos.x + (crop.left * scale.x), pos.y + (crop.top * scale.y) + height - 4.0f, 0.0f);
  gs_draw_sprite(nullptr, 0, width, 4.0f);
  gs_matrix_pop();
  ...
  // Right border
  gs_matrix_push();
  gs_matrix_translate3f(pos.x + (crop.left * scale.x) + width - 4.0f, pos.y + (crop.top * scale.y), 0.0f);
  gs_draw_sprite(nullptr, 0, 4.0f, height);
  gs_matrix_pop();

  // Dragging point box (25x25 pixels in bottom-right corner)
  gs_matrix_push();
  gs_matrix_translate3f(pos.x + (crop.left * scale.x) + width - 25.0f, pos.y + (crop.top * scale.y) + height - 25.0f, 0.0f);
